...
def loans_test():
    ...
    # Additional tests for Applicant Class
    applicant = loans.Applicant("40-50", ["2", "3"])
    assert applicant.race == {"Asian", "Black or African American"}
    loans_points += 1

    applicant = loans.Applicant("60-70", ["1", "4", "5"])
    assert applicant.race == {"American Indian or Alaska Native", "Native Hawaiian or Other Pacific Islander", "White"}
    loans_points += 1

    applicant = loans.Applicant("70+", ["21", "22", "23", "24", "25", "26", "27"])
    assert applicant.race == {"Asian Indian", "Chinese", "Filipino", "Japanese", "Korean", "Vietnamese", "Other Asian"}
    loans_points += 1
